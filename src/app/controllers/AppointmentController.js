...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    if(req.userId == provider_id) {
      return res.status(401).json({ error: 'Um prestador de serviço não pode fazer agendamentos'})
    }
    ...
    //Refazer usando MYSQL
	await Notification.create({
      content: `Novo agendamento de ${user.name} para ${formattedDate}`,
      user: provider_id,      
    });
	const {id} = appointment;
	const appointment_mail = await Appointment.findByPk(id, {
      include: [
        {
          model: User,
          as: 'provider',
	...
	// envia email de aviso de agendamento
  ...
  async delete(req, res){
	...
	if (!appointment){
		return res.status(404).json({error: "O agendamento solicitado não foi localizado"});
	}
